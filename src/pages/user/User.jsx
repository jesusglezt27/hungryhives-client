...
const User = () => {
  ...
  const fetchReservations = () => {
    fetch(`https://hungryhives-server-5.vercel.app/api/restaurants/user/${user._id}/reservations`)
      .then(response => {
        if (response.ok) {
  ...
  const handleDelete = (id) => {
    fetch(`https://hungryhives-server-5.vercel.app/api/restaurants/reservations/${id}`, {
      method: 'DELETE'
    })
      .then(response => {
        if (response.ok) {
